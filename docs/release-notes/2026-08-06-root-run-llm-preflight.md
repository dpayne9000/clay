...
Every root workflow now checks the configured model endpoint before executing
its first action. A missing server fails immediately through the existing
`run.error` event and explains the exact `llama-server --hf-repo ...` and
`curl .../health` commands needed to start and verify the configured model.
